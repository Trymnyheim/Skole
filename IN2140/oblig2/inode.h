...
/* This function frees the entries field of the inode pointer node.
 * If node is a directory, it also calls fs_shutdown on all its child
 * inodes.
 */
void free_entries( struct inode* node );

/* This function adds a new inode to the parent directory's entries field.
 */
void add_inode_to_parent( struct inode* parent, struct inode* child);

/* This function removes a specific inode from the parent directory's entries
 * field
 */
void remove_inode_from_parent(struct inode *parent, struct inode *child);

/* This functions saves all inodes recursively to disk
 */
void rec_save_inodes(FILE* f, struct inode* node);

/* This function saves a specific inode to disk
 */
